...
ytrain = (ytrain1 - meany1) / stdy1 * alpha - (ytrain2 - meany2) / stdy2 * (1 - alpha)
xtest = df.loc[Ntstart:Ntend, :3].values
ytest = (ytest1 - meany1) / stdy1 * alpha - (ytest2 - meany2) / stdy2 * (1 - alpha)
...
param_ft = {}
param_ft['par_bar'] = par_bar
param_ft['xbound'] = xbound
param_ft['gp'] = gp
np.save('param_ft.npy', param_ft)
